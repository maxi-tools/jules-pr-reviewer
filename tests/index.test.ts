...
describe('index.ts', () => {
  ...
  beforeEach(async () => {
    ...
    mockJulesHelper.runJulesReview.mockResolvedValue({
      reviewResult: { verdict: 'approve', summary: 'Good job', newComments: [] },
  ...
  it('fails if eventName is pull_request_target', async () => {
    ...
    await loadIndex();
    expect(mockSetFailed).toHaveBeenCalledWith(expect.stringContaining('pull_request_target is not supported'));
  });

  it('fails if eventName is not pull_request', async () => {
    (github as any).context.eventName = 'push';
    await loadIndex();
    expect(mockSetFailed).toHaveBeenCalledWith(expect.stringContaining('Unsupported event'));
  });

  it('fails if no pull_request payload', async () => {
    (github as any).context.payload.pull_request = undefined;
    await loadIndex();
    expect(mockSetFailed).toHaveBeenCalledWith('No pull_request payload found.');
  });
  ...
  it('fails if fail_on is invalid', async () => {
    ...
    await loadIndex();
    expect(mockSetFailed).toHaveBeenCalledWith(expect.stringContaining('Invalid fail_on'));
  });

  it('skips draft PR if skip_drafts is true', async () => {
    (github as any).context.payload.pull_request.draft = true;
    mockGetBooleanInput.mockImplementation((name: string) => name === 'skip_drafts');
    await loadIndex();
    expect(mockInfo).toHaveBeenCalledWith('Skipping draft PR.');
  });

  it('skips fork PR if skip_forks is true', async () => {
    (github as any).context.payload.pull_request.head.repo.full_name = 'fork/repo';
    mockGetBooleanInput.mockImplementation((name: string) => name === 'skip_forks');
    await loadIndex();
    expect(mockInfo).toHaveBeenCalledWith('Skipping fork PR (skip_forks=true).');
  });
  ...
  it('skips if bypass label is present', async () => {
    ...
    (github as any).context.payload.pull_request.labels = [{ name: 'skip-review' }];
    await loadIndex();
    expect(mockInfo).toHaveBeenCalledWith('Bypass label "skip-review" present — skipping review.');
  });
  ...
  it('uses ctx.payload.before for diff on synchronize event', async () => {
    ...
    expect(mockGithubHelper.fetchDiff).toHaveBeenCalledWith(
      expect.anything(), 'owner', 'repo', expect.anything(), 'beforeSHA', 'headSHA'
    );
  });
  ...
  it('loads rules if rules_file is provided', async () => {
    ...
    expect(mockGithubHelper.loadRulesFromBase).toHaveBeenCalledWith(
      expect.anything(), 'owner', 'repo', 'rules.md', 'baseSHA'
    );
  });
  ...
  it('truncates large diffs', async () => {
    ...
    expect(mockJulesHelper.runJulesReview).toHaveBeenCalledWith(
      expect.anything(),
      expect.stringContaining('NOTE: The diff was truncated: original 81000 chars, kept first 80000.'),
      expect.anything(),
      expect.anything()
    );
  });
  ...
  it('handles Jules failure to return review', async () => {
    mockJulesHelper.runJulesReview.mockResolvedValue({ reviewResult: null, sessionId: 's1' });
    await loadIndex();
    expect(mockGithubHelper.setStatus).toHaveBeenCalledWith(
      expect.anything(), 'owner', 'repo', 'headSHA', expect.anything(), 'error', 'Jules did not return a valid review in time'
    );
    expect(mockSetFailed).toHaveBeenCalledWith(expect.stringContaining('Jules returned no review message'));
  });
  ...
  it('resolves open threads if resolvedCommentIds provided', async () => {
    ...
    mockJulesHelper.runJulesReview.mockResolvedValue({
      reviewResult: { verdict: 'approve', summary: 'ok', resolvedCommentIds: [2] },
      sessionId: 's1'
    });
    await loadIndex();
    expect(mockGithubHelper.resolveThreads).toHaveBeenCalledWith(expect.anything(), ['t2']);
  });
  ...
  it('submits review and sets status based on verdict', async () => {
    ...
    expect(mockGithubHelper.setStatus).toHaveBeenCalledWith(
      expect.anything(), 'owner', 'repo', 'headSHA', expect.anything(), 'failure', 'Blocking issues found'
    );
  });
  ...
  it('handles fail_on = never', async () => {
    ...
    await loadIndex();
    expect(mockGithubHelper.setStatus).toHaveBeenCalledWith(
      expect.anything(), 'owner', 'repo', 'headSHA', expect.anything(), 'success', 'Review complete (verdict: block)'
    );
  });
  ...
  it('handles fail_on = any with approve verdict', async () => {
    ...
    await loadIndex();
    expect(mockGithubHelper.setStatus).toHaveBeenCalledWith(
      expect.anything(), 'owner', 'repo', 'headSHA', expect.anything(), 'success', 'Approved'
    );
  });

  it('fails immediately when initial setStatus throws permission error', async () => {
    mockGithubHelper.setStatus.mockRejectedValueOnce(new Error('Initial setStatus failed'));
    mockJulesHelper.wrapPermissionError.mockReturnValueOnce(new Error('Wrapped initial setStatus failed'));
    await loadIndex();
    // It should have caught the error, wrapped it, and then caught it in the top level catch.
    expect(mockSetFailed).toHaveBeenCalledWith('Jules PR review failed: Wrapped initial setStatus failed');
  });

  it('top-level catch works when run throws synchronously', async () => {
    // If core.getInput throws, run() will reject before async things
    mockGetInput.mockImplementation(() => { throw new Error('Sync error'); });
    await loadIndex();
    expect(mockSetFailed).toHaveBeenCalledWith('Sync error');
  });

  it('top-level catch handles non-Error objects', async () => {
    mockGetInput.mockImplementation(() => { throw 'String error'; });
    await loadIndex();
    expect(mockSetFailed).toHaveBeenCalledWith('String error');
  });

  it('handles exception in the process', async () => {
    mockGithubHelper.fetchDiff.mockRejectedValue(new Error('Fetch diff failed'));
    await loadIndex();
    expect(mockSetFailed).toHaveBeenCalledWith('Jules PR review failed: Fetch diff failed');
    expect(mockGithubHelper.setStatus).toHaveBeenCalledWith(
      expect.anything(), 'owner', 'repo', 'headSHA', expect.anything(), 'error', 'Fetch diff failed'
    );
  });
});
