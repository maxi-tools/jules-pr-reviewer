...
import { runJulesReview, isAuthError, wrapPermissionError } from '../src/jules.js';
...
describe('jules.ts', () => {
  ...
  describe('runJulesReview', () => {
    it('returns null if no reviewMessage is collected', async () => {
      ...
      expect(result).toEqual({ reviewResult: null, sessionId: 'test-session-id' });
    });
    ...
    it('fails immediately when session.info() fails with non-auth, non-404 error', async () => {
      const sessionInfoMock = vi.fn()
        .mockRejectedValueOnce(new Error('500 server error'));
      
      const mockSession = mockSessionWithHistory([{ type: 'agentMessaged', message: '{"summary":"test","verdict":"approve"}' }]);
      ...
      await expect(runJulesReview('api-key', 'prompt', {}, 1)).rejects.toThrow('Jules session.info() failed: 500 server error');
    });
    ...
    it('fails when session.info() throws auth error', async () => {
      ...
      mockSession.info = vi.fn().mockRejectedValue(new Error('401 Unauthorized'));
      ...
      await expect(runJulesReview('api-key', 'prompt', {}, 1)).rejects.toThrow('Jules API rejected request (401 Unauthorized). Check JULES_API_KEY is valid.');
    });
    ...
    it('fails when session.info() fails max attempts', async () => {
      ...
      const promise = expect(runJulesReview('api-key', 'prompt', {}, 1)).rejects.toThrow('Session did not become ready within timeout.');
    ...
    it('handles hydrate failure with string error and non-agentMessaged event', async () => {
      ...
      const mockSession = mockSessionWithHistory([
        ...
        { type: 'agentMessaged', message: '{"summary":"test","verdict":"approve"}' }
      ]);
    ...
    it('fails when hydrate throws auth error', async () => {
      const mockSession = mockSessionWithHistory([]);
      mockSession.hydrate = vi.fn().mockRejectedValue(new Error('403 Forbidden'));
      ...
      const promise = runJulesReview('api-key', 'prompt', {}, 1);
      await expect(promise).rejects.toThrow('Jules API rejected request (403 Forbidden). Check JULES_API_KEY is valid.');
    });
  });
  ...
  describe('isAuthError & wrapPermissionError', () => {
    ...
    it('wraps 403 error with helpful instructions', () => {
      ...
      const result = wrapPermissionError(err, 'statuses:write', 'createCommitStatus');
