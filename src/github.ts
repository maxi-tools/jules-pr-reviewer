...
export async function fetchDiff(
  ...
  owner: string, repo: string, pr: any, baseShaForDiff: string, headSha: string
): Promise<string> {
  try {
    ...
    core.warning(`compareCommitsWithBasehead failed, falling back to pulls.get: ${String(err)}`);
  }
...
export async function fetchOpenThreads(
  ...
  `;
  const response: any = await octokit.graphql(query, { owner, repo, pr: prNumber });
